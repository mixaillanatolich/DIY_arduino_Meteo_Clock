...
void redrawPlot() {
  ...
  switch (mode) {             // добавлена переменная для "растягивания" графика до фактических максимальных и(или) минимальных значений(с)НР
    case 1: drawPlot(0, 3, 15, 4, TEMP_MIN, TEMP_MAX, (int*)tempHour, "t" + String(char(223)) + "hr", mode);
      break;
    case 2: drawPlot(0, 3, 15, 4, TEMP_MIN, TEMP_MAX, (int*)tempDay, "t" + String(char(223)) + "day", mode);
      break;
    ...
    case 4: drawPlot(0, 3, 15, 4, HUM_MIN, HUM_MAX, (int*)humDay, "h%day", mode);
      break;
    case 5: drawPlot(0, 3, 15, 4, PRESS_MIN, PRESS_MAX, (int*)pressHour, "p hr", mode);
      break;
    case 6: drawPlot(0, 3, 15, 4, PRESS_MIN, PRESS_MAX, (int*)pressDay, "p day", mode);
      break;
    case 7: drawPlot(0, 3, 15, 4, CO2_MIN, CO2_MAX, (int*)co2Hour, "c hr", mode);
      break;
    case 8: drawPlot(0, 3, 15, 4, CO2_MIN, CO2_MAX, (int*)co2Day, "c day", mode);
      break;
  }
}
...
void drawSensors() {
  ...
  if (mode0scr != 2) {                      // Температура (с)НР ----------------------------
    lcd.setCursor(0, 2);
    if (bigDig && mode0scr == 1) lcd.setCursor(15, 2);
    if (bigDig && mode0scr != 1) lcd.setCursor(15, 0);
